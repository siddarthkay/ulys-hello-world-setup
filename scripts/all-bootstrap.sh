...
#   9. print URLs
#
# Postgres + Redis run as containers on the VM (compose.infra.yml). Postgres
# reads its initial password from the DB_PASSWORD env var on first boot,
# which deploy-tools fetches from Secret Manager. No managed Cloud SQL /
# Memorystore to wait on.
...
for s in "$DB_SECRET" "$WS_SECRET"; do
  gcloud secrets describe "$s" --project "$PROJECT_ID" >/dev/null 2>&1 || {
    echo "  creating secret container: $s"
    gcloud secrets create "$s" --project "$PROJECT_ID" --replication-policy=automatic >/dev/null
  }
done

DB_PASS=$(openssl rand -base64 32 | tr -d '=+/' | head -c 32)
WS_VAL=$(openssl rand -hex 32)

printf '%s' "$DB_PASS" | gcloud secrets versions add "$DB_SECRET" --data-file=- --project "$PROJECT_ID" >/dev/null
printf '%s' "$WS_VAL"  | gcloud secrets versions add "$WS_SECRET" --data-file=- --project "$PROJECT_ID" >/dev/null
echo "  added new versions to $DB_SECRET, $WS_SECRET"
...
# --- 7. Write main tfvars ---------------------------------------------------
# Only per-project facts live here. Tunables (name_prefix, vm_machine_type,
# pgdata_disk_gb) keep their defaults in variables.tf. Zone is derived from
# region in locals.tf so it doesn't appear here either.
...
# --- 8. Apply main TF -------------------------------------------------------
log "terraform apply (main): VPC, VM (with attached pgdata disk), Artifact Registry. ~2 min."
